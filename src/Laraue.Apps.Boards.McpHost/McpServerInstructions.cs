...
public static class McpServerInstructions
{
    public const string Text =
        ...
        "issue's content, delete_issue to remove one, and create_comment/edit_comment for its " +
        "comments. get_issue's comment list includes each comment's id, needed by edit_comment - " +
        "only the comment's own author can edit it. edit_issue_status/create_issue take a status " +
        "id, not a name - call list_statuses first to find it. Before calling create_issue/" +
        ...
        "issue - check these before calling edit_issue/edit_issue_status/delete_issue on it, " +
        "rather than finding out from a permission error. list_spaces returns canCreateIssue per " +
        "space - check it before calling list_statuses/create_issue there.";
}
