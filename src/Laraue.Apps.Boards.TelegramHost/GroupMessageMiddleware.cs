...
/// messages into cards. Runs after <see cref="HandleAllMessagesMiddleware"/> so it can
...
/// </summary>
public class GroupMessageMiddleware(
    RequestContext context,
    IGroupChatCardService groupChatCardService)
    : ITelegramMiddleware
{
    ...
    public async Task InvokeAsync(Func<CancellationToken, Task> next, CancellationToken ct)
    {
        ...
        await groupChatCardService.HandleGroupMessage(message, ct);

        context.SetExecutedRoute(new ExecutedRouteInfo(nameof(GroupMessageMiddleware), message.Text));
    }
}
