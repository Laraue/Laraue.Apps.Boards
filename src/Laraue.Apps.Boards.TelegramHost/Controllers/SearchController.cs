    ...
    : TelegramController
{
    ...
    {
        var readableSpaceIds = await GetReadableSpaceIdsAsync(context, requestContext, ct);
        var inlineQuery = requestContext.Update.InlineQuery!;

        logger.LogDebug(
            "Inline search by user {UserId}: raw query {RawQuery:l}",
            requestContext.UserId, inlineQuery.Query);

        if (readableSpaceIds.Length == 0)
        {
            await AnswerNoResults(
                botClient, inlineQuery.Id, "no-spaces",
                "No accessible spaces", "You don't have read access to any spaces yet.", ct);
            return;
        }
            ...
            switch (resolution)
            {
                ...
                case SuggestionsResolution suggestions:
                    await botClient.AnswerInlineQuery(
                        inlineQuery.Id, suggestions.Results, cacheTime: 0, cancellationToken: ct);
                    return;

                case ErrorResolution error:
                    await AnswerNoResults(
                        botClient, inlineQuery.Id, $"{token.Key}-error", error.Title, error.Message, ct);
                    return;
            }
        }
        ...
        {
            await AnswerNoResults(
                botClient, inlineQuery.Id, "no-issues", "No issues found",
        ...
        await botClient.AnswerInlineQuery(inlineQuery.Id, result, cacheTime: 0, cancellationToken: ct);
    }
    ...
    private static async Task AnswerNoResults(
        ITelegramBotClient botClient,
        ...
        CancellationToken ct)
    {
        ...
        await botClient.AnswerInlineQuery(
            inlineQueryId, [placeholder], cacheTime: 0, cancellationToken: ct);
    }
}