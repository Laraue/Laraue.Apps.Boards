...
{
    private readonly Proxy<UserController> _controller = host.Controller<UserController>();

    [Fact]
    public async Task User_ShouldStoreOnboardingStatus_ByOnboardingId()
    {
        ...
        var initial = await _controller
            ...
            .Execute(x => x.GetOnboardingStatus(OnboardingId.AppLayoutV1, default));
        Assert.Null(initial.Status);

        await _controller
            .WithUserAuthorization(userId)
            .Execute(x => x.SetOnboardingStatus(
